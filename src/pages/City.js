...
import {addCity, deleteCity, getAllCities, updateCity} from "../utils/apiFunctions/cityApiFunctions";
...
function City() {
    ...
    //update declaration
    const [cityIdUpdate, setCityIdUpdate] = useState(0);
    ...
    const openAddEditModal = (op,id,name,description) => {
        ...
        if (op===1){
            ...
            setModalTitle("Créer une ville");
        }
        else if (op===2){
            setOperation(2);
            setShowSaveEditModal(true);
            setModalTitle("Modifier une ville");
            setCityIdUpdate(id);
            setNewCity({
                "name":name,
    ...
    const handleSubmit = (e) => {
        ...
        if (operation === 1) {
            saveNewCity().then(() => console.log("Company saved successfully"))
                .catch((error) => console.error("Error in handleSubmit:", error));
        } else {

            updateGivenCity().then(() => console.log("Company updated successfully"))
                .catch((error) => console.error("Error in handleSubmit:", error));
        }
    };
    ...
    const saveNewCity = async () => {
        ...
        setIsSaving(true);
        try {
            const success = await addCity(newCity.name, newCity.description);
    ...
    const updateGivenCity = async () => {

        setIsSaving(true);
        try {
            const success = await updateCity(cityIdUpdate,newCity.name, newCity.description);
    ...
    const handleDelete = async (e) => {
        setIsDeleting(true);
        try {
            const result = await deleteCity(idCity);
                                                                ...
                                                                <tr key={company.id}>
                                                                    ...
                                                                    <td className="action-table-data">
                                                                        <div className="edit-delete-action">
                                                                            ...
                                                                            <a
                                                                                onClick={(e) =>  openAddEditModal(2,company.id,company.name,company.description) }
                                                                                className="me-2 p-2">
                                                                                <svg
                                                                                    xmlns="http://www.w3.org/2000/svg"
                                                                                    width="24" height="24"
